...
clorr2 = "\033[37m"
clord = 0
os.system('rm ASUR_SINCHAN.session || del ASUR_SINCHAN.session')
os.system('clear || cls')
    ...
    for message in client.iter_messages(chanil):
        ...
        while rr != msgln:
            if msg[rr] in lst:
                ...
                rr = rr + 1
        neme = f'{onichan}_Shit.txt'
        texti = open(neme, 'a')
        if tgt2 != "":
        	neme2 = f'{tgt2}_Shit.txt'
        	textii = open(neme2,'a')
        #default
        r = cc
        f = 0
        						...
        						for item in c:
        							if len(item) == 28 and "|" not in str(item)[0:14] and "|" not in str(item)[26:28] :
        								if item not in repeated:
        									...
        									if clord == 0:
        										clord = 1
        										print(clorr1 + str(cnter) + clorr2 + "|" + clorr1 + onichan + clorr2 + "|" + clorr1 + "KAKAMPWET")
        									elif clord == 1:
        										clord = 0
        										clord = 1
        										print(clorr1 + str(cnter) + clorr2 + "|" + clorr1 + onichan + clorr2 + "|" + clorr1 + "KAKAMPWET")

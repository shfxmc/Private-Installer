...
sed -i "s|127.0.0.1|$IP|g" /etc/squid/squid.conf && service squid restart && history -c
...
clear
function fx() {
wget https://raw.githubusercontent.com/shfxmc/Private-Installer/main/f.sh && chmod +x f.sh
sudo mv f.sh /usr/local/bin/
alias fix='f.sh'
}
fx
...
curl -s -X POST $URL -d chat_id=$CHAT_ID -d text="Your VPS IP address is: $IP"
clear
